...
Be clear and concise in your decision.`,
  ...
  async function formatterNode(
    ...
    const creatorMessage = state.messages.find(
      (msg) => msg.name === "QuestionCreator"
    );
    const analysisMessage = state.messages.find(
      (msg) => msg.name === "QuestionAnalysis"
    );
    ...
    const headerPart = originalMessage
      ? (originalMessage.content as string).split("Content to generate questions from:")[0]
      : "";
...
Question Paper Details: ${headerPart}

Original questions: ${creatorMessage?.content ?? ""}

Analysis and improvements: ${analysisMessage?.content ?? ""}

Create a well-structured, professional exam paper that incorporates all the feedback and improvements.
Output ONLY the final exam paper. Do NOT include analysis, validation notes, internal reasoning, safety commentary, or meta-discussion.`,
      }),
    ];
  ...
  function mainRouter(state: typeof AgentState.State) {
    ...
    if (lastMessage.name === "Extractor") {
      ...
      if (content.includes("NOT PERFECT") && iterationCount < 2) {
